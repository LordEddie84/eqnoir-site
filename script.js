async function fetchMarketData() {
  try {
    ...
    const volume = parseFloat(pool.volume_24h_usd).toFixed(2);
    const liquidity = parseFloat(pool.liquidity_usd).toFixed(2);
    const holders = pool.holders_count;

    const marketData = `🪙 EQNOIR / WBNB • Price: $${price} • 24h Volume: $${volume} • Liquidity: $${liquidity} • Holders: ${holders}`;
